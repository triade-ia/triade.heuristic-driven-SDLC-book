...
Gera documentação OpenAPI 3.0 a partir de **requisitos textuais** (Modo 1) ou **código-fonte existente** (Modo 2). O output é sempre salvo em `swagger/<nome_normalizado>/` com `openapi.yaml` e `README.md`.
...
- Criar `swagger/` e o subdiretório se não existirem.
...
- Criar `swagger/<nome_funcionalidade>/`.
...
- Normalizar e criar `swagger/<nome_projeto>/`.
...
**Input:** Arquivo como `setUp/REQ_FINAL.MD` com título "Especificação Técnica: Envio de QualiPoints (v1.0)".

**Output:** `swagger/envio_qualipoints/openapi.yaml` e `README.md`.
...
1. **Confirmação**: Listar paths completos dos arquivos criados (ex.: `swagger/envio_qualipoints/openapi.yaml`, `swagger/envio_qualipoints/README.md`).
2. **Resumo da análise**:
   - Total de endpoints
