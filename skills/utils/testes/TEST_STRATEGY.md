---
name: test-strategy
description: Analisa requisitos e código para sugerir estratégia de testes. Identifica heurísticas aplicáveis, define distribuição por camada, gera casos de teste e SALVA RELATÓRIO em output/. Use quando precisar decidir QUAIS testes criar e ONDE. Sempre gere relatório estruturado ao finalizar análise.
---
...
1. **Identifique heurísticas aplicáveis** (Baica, VADER, FAILURE, CRUD, Count, Chique, SeenAndHeard, InputMethod)
...
4. **Crie relatório estruturado** e salve em `output/test-strategy-[nome-funcao]-[YYYYMMDD].md`
5. **Recomende próximos passos** (qual guide consultar e como passar o relatório)

**IMPORTANTE**: Ao finalizar a análise, SEMPRE gerar o arquivo de relatório em `output/` para que o desenvolvedor possa usá-lo como contexto ao chamar os guides.

## Processo de Análise

### 1. Identificação de Heurísticas

Para cada requisito/código, avalie quais heurísticas aplicar:
...
| Quantidades variáveis (0, 1, N itens) | **Count** |
| Interface do usuário, formulários | **Chique** (campos obrigatórios, habilitar/desabilitar) |
| Feedback visual, acessibilidade, mensagens | **SeenAndHeard** |
| Métodos de entrada (digitação, colar, import, drag/drop) | **InputMethod** |
...
- Componente React/Vue → Chique + SeenAndHeard + InputMethod
- Fluxo completo usuário → FAILURE (Emotions, Recovery) + E2E
...
- **ID único**: UT-XXX-01, IT-XXX-01, API-XXX-01, E2E-XXX-01
...
**Caminho**: `output/test-strategy-[nome-funcao]-[YYYYMMDD].md`

**Exemplo de nome**: `output/test-strategy-validateAmount-20260217.md`
...
2. Que o relatório deve ser passado junto: `@TEST_UNIT_GUIDE.md @output/test-strategy-xxx.md`
3. Quais casos implementar (ex: "UT-VAL-01 a UT-VAL-07")
4. Arquivos sugeridos (ex: `tests/unit/validators/validateAmount.test.ts`)
...
- [ ] Baica (Boundaries, Nulls, Special Chars, Formats)
- [ ] VADER (Values, Authorization, Data Consistency, Error Handling)
- [ ] FAILURE (Functional, Appropriate, Recovery, Emotions)
- [ ] CRUD (Create, Read, Update, Delete)
- [ ] Count (0, 1, Muitos)
- [ ] Chique (campos obrigatórios, estados)
- [ ] SeenAndHeard (feedback, acessibilidade)
- [ ] InputMethod (typing, paste, import)
...
## Referências
- Heurísticas: [links para skills Baica, VADER, etc.]
- Guides: [links para TEST_UNIT_GUIDE, etc.]

---
**IMPORTANTE**: Use este relatório como contexto ao chamar os guides!

Exemplo: `@TEST_UNIT_GUIDE.md @output/test-strategy-[este-arquivo].md`
...
| Componente de formulário | Chique, SeenAndHeard, InputMethod | Componentes (90%), E2E (10%) |
| Fluxo completo (ex: login → transferir) | FAILURE, SeenAndHeard | E2E (70%), Serviço (30%) |
...
4. **Relatório**: Gerar `output/test-strategy-validateAmount-20260217.md` com os 7 casos
5. **Recomendação**: "Consulte TEST_UNIT_GUIDE.md passando o relatório. Implemente UT-VAL-01 a UT-VAL-07 em tests/unit/validators/validateAmount.test.ts"
...
1. Salve o relatório em `output/` (a skill deve tê-lo gerado)
2. Chame o guide com o relatório: `@TEST_UNIT_GUIDE.md @output/test-strategy-validateAmount-20260217.md`
...
- [ ] Todas as heurísticas aplicáveis foram identificadas?
- [ ] A distribuição por camada segue a pirâmide (mais unitários, menos E2E)?
- [ ] Cada caso tem ID único, input, output e prioridade?
- [ ] O relatório foi gerado e salvo em `output/test-strategy-[nome]-[data].md`?
- [ ] O relatório segue o template (heurísticas, casos, próximos passos)?
