...
- **Ação**: ler arquivos com Read/Glob; identificar entidades, serviços, endpoints e operações de dados
- **Extrair**: modelos de dados, controllers, repositórios, validações
- Usar como base para o Processo de Análise abaixo

### Tipo 4 — Função específica
- **Detectar**: input é um caminho de arquivo com função específica (ex: `src/services/order.service.ts#createOrder`) ou bloco de código inline (` ``` `)
...
- **Extrair**: operação CRUD realizada, validações, dados manipulados, dependências
- Usar como base para o Processo de Análise abaixo
...
- **Cruzar**: verificar se operações CRUD da ferramenta de gestão estão implementadas no código e vice-versa
- Usar a visão combinada como base para o Processo de Análise abaixo

> **Nota:** Ao iniciar o relatório, inclua uma linha identificando a fonte analisada. Exemplo: `**Fonte analisada:** Story #abc123 — Cadastro de Produto + src/modules/products/`
