...
|------|---------------|-----------------|
| **Épico** | ID/título do épico na ferramenta de gestão | Identificação macro das áreas de risco; gaps de especificação de regressão no épico |
| **Épico + Tarefa** | ID do épico + ID/título da story ou tarefa | Análise RCRCRC sobre as mudanças e funcionalidades especificadas na tarefa |
| **Repositório** | URL ou nome do repositório + branch | Análise do código modificado: Recent, Risky e Complex identificados diretamente no código |
| **Função Específica** | Trecho de código ou nome da função + arquivo | Análise cirúrgica das 6 dimensões RCRCRC no ponto de mudança |
| **Combinado** | Qualquer combinação dos anteriores | Análise completa: escopo da mudança → código → priorização de regressão |
...
- Leia o épico e identifique as funcionalidades e módulos envolvidos
- Para cada área identificada, avalie: quão recente é, se é Core, histórico de riscos, sensibilidade a configuração, conformidade e complexidade
- Saída: mapa de risco macro com as 6 dimensões e perguntas para as tasks filhas

### Épico + Tarefa → Análise de Regressão Focada
- Use o épico para contexto de negócio; use a tarefa para o escopo exato da mudança
- Aplique as 6 dimensões RCRCRC sobre as funcionalidades afetadas pela tarefa
- Saída: análise completa com áreas priorizadas, lacunas de cobertura e recomendações de teste

### Repositório → Análise do Código Modificado
- Inspecione o histórico de commits e os arquivos alterados no repositório
- Para cada módulo modificado, aplique as 6 dimensões RCRCRC (especialmente Recent, Risky e Complex)
- Saída: análise do código real com módulos adjacentes afetados e gaps de automação identificados

### Função Específica → Análise Cirúrgica
- Foque na função/módulo fornecido e aplique as 6 dimensões de forma cirúrgica
- Identifique qual dimensão representa maior risco no ponto de mudança
- Saída: análise pontual com top riscos de regressão e plano de teste concreto

### Combinado → Análise Completa
- Execute Análise de Risco (épico/tarefa) + Análise de Código (repositório/função) em sequência
- Consolide em relatório único: escopo da mudança → dimensões RCRCRC → priorização → decisão de deploy
